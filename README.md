...
- セーブ内死亡数とキャラクター別NG+オフセットを合計した累計死亡数の表示
- 撃破数、未撃破数、総数、進捗率の表示
- 地域別進捗とボス一覧の連動
...
保存するのはセーブファイルのパス、キャラクタースロット番号、表示言語（`ja` / `en`）、自動監視の有効状態、画面テーマ（`dark` / `light`）、OBS出力の有効状態・出力先・`progress.txt`の表示書式、キャラクター別の死亡数オフセットです。死亡数オフセットはセーブファイルのパスとスロット番号の組み合わせに紐づけます。セーブ内容、イベントフラグ、キャラクター名は設定へ保存しません。設定を書き込めない場合は設定タブ下部へ警告を表示し、画面上の変更は維持したまま次の設定変更時に保存を再試行します。

## 累計死亡数

表示する累計死亡数は`セーブ内死亡数 + NG+オフセット`です。設定タブで選択中のキャラクターに対するオフセットを入力し、［オフセットを適用］を押すと画面とOBS出力へ反映されます。過去周回分はセーブから自動推測せず、利用者が確認した値を入力します。オフセットを`0`にして適用すると、そのキャラクターの保存済みオフセットを解除します。

## OBSテキスト出力

［OBSテキスト出力］をオンにすると、既定では`%LOCALAPPDATA%\ERBossTrackerJP\OBS`へ`progress.txt`、`deaths.txt`、`defeated.txt`、`remaining.txt`、`total.txt`、`percentage.txt`、`latest_boss.txt`、`snapshot.json`を出力します。出力先は画面から変更できます。OBS出力タブには`progress.txt`と`deaths.txt`のパスを用途別に表示し、ボタンからコピーできます。
