...
6. OBSへ表示する場合は［OBS出力］タブの［OBSテキスト出力を有効にする］をオンにし、OBSの「テキスト（GDI+）」で出力ファイルを選択します。

画面はダークモードで起動します。［設定］タブの［ダークモード］をオフにするとライトモードへ即時に切り替わり、選択は次回起動時にも復元されます。
...
- 最後に読み込んだセーブ、キャラクタースロット、表示言語、自動監視設定、画面テーマ、OBS出力設定、ウィンドウ位置・サイズの復元
...
保存するのはセーブファイルのパス、キャラクタースロット番号、表示言語（`ja` / `en`）、自動監視の有効状態、画面テーマ（`dark` / `light`）、OBS出力の有効状態・出力先・`progress.txt`の表示書式、キャラクター別の周回基準値とON／OFF状態、ウィンドウ位置・サイズ・最大化状態です。周回基準値はセーブファイルのパスとスロット番号の組み合わせに紐づけます。最小化状態は保存せず、次回起動時に最小化したままになりません。保存位置のモニターが取り外されている場合は、利用可能な画面内へウィンドウを戻します。

ボス名検索、撃破状態、本編／DLC、地域フィルター、進捗タブの地域選択はセッション限定で、再起動時に初期状態へ戻します。セーブ内容、イベントフラグ、キャラクター名も設定へ保存しません。設定を書き込めない場合は設定タブ下部へ警告を表示し、画面上の変更は維持したまま次の設定変更時に保存を再試行します。

## 累計死亡数

ELDEN RINGのセーブ内死亡数は、New Gameの周回をまたいだ累計値です。進捗タブの死亡数カード内に、この補正前の累計値を表示します。

［設定］または［OBS出力］タブで周回オフセットを有効にすると、OBSなどに使う表示死亡数を`max(0, セーブ内累計死亡数 - 周回開始時の基準値)`で計算します。［現在値を基準にする］を押すと、その時点の累計死亡数を保存して表示死亡数を`0`から開始できます。基準値は直接入力でき、ON／OFFを切り替えても保存値を維持します。基準値が累計死亡数を上回った場合は表示死亡数を`0`に制限して警告します。

誤った加算方式を使用していた設定スキーマ1の死亡数オフセットは、安全のため基準値へ変換せず破棄します。セーブパス、表示、監視、OBSなどの他の設定は引き継ぎます。

## OBSテキスト出力

［OBSテキスト出力を有効にする］をオンにすると、既定では`%LOCALAPPDATA%\ERBossTrackerJP\OBS`へ`progress.txt`、`deaths.txt`、`defeated.txt`、`remaining.txt`、`total.txt`、`percentage.txt`、`latest_boss.txt`、`snapshot.json`を出力します。`deaths.txt`には周回基準値を反映した表示死亡数を出力します。出力先は画面から変更でき、OBS出力タブでは`progress.txt`と`deaths.txt`のパスを用途別にコピーできます。
