...
現在はセーブ解析、207件のボスデータ、撃破判定・集計、WPF画面への進捗表示、自動監視、OBSテキスト出力、ダーク／ライトテーマ、主要なユーザー設定の保存と復元、診断ログまで実装済みです。
...
保存するのはセーブファイルのパス、キャラクタースロット番号、表示言語（`ja` / `en`）、自動監視の有効状態、画面テーマ（`dark` / `light`）、OBS出力の有効状態・出力先・`progress.txt`の表示書式です。セーブ内容、イベントフラグ、キャラクター名は設定へ保存しません。設定を書き込めない場合は設定タブ下部へ警告を表示し、画面上の変更は維持したまま次の設定変更時に保存を再試行します。

## OBSテキスト出力

［OBSテキスト出力］をオンにすると、既定では`%LOCALAPPDATA%\ERBossTrackerJP\OBS`へ`progress.txt`、`defeated.txt`、`remaining.txt`、`total.txt`、`percentage.txt`、`latest_boss.txt`、`snapshot.json`を出力します。出力先は画面から変更できます。
